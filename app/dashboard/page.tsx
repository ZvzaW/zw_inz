  ...
  const loadNotifications = async (pageNum: number, isBackgroundRefresh = false) => {
    ...
    try {
      ...
      setNotificationsGrouped(prev => {
        const newState = { ...prev }
        Object.entries(grouped as Record<string, any[]>).forEach(([label, items]) => {
          if (newState[label]) {
                      ...
                      {items.map((notif: any) => (
                        <button
                          ...
                          className={`bg-dirty-blue hover:bg-hover group flex w-full items-center justify-between rounded-xl p-4 text-left transition-all ${
                            !notif.is_read ? "border-baby-blue border-2" : "border-zinc-800 border" 
                          }`}
                        >
                ...
                {hasMore && !error && (
                  <div className="flex justify-center pb-4">
                    <button 
                      ...
                      className="px-4 py-3 text-baby-blue text-sm hover:bg-dark-navy/70 bg-dirty-navy/70 rounded-lg"
                    >
                        {isFetchingMore ? <Loader2 className="animate-spin"/> : "Załaduj więcej"}
                    </button>
                  </div>
                )}
              </>
